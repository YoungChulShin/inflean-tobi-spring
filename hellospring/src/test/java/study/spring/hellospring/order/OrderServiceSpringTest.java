...
import study.spring.hellospring.OrderConfig;
...
public class OrderServiceSpringTest {
  ...
  void createDuplicatedOrders() {
    ...
    Assertions.assertThatThrownBy(() -> orderService.createOrders(orderReqs)).isInstanceOf(
        DataIntegrityViolationException.class);

    JdbcClient client = JdbcClient.create(dataSource);
    var count = client.sql("select count(*) from orders where no = '0300'").query(Long.class)
        .single();
